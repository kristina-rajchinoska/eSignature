...
export const createTemplate = async (args) => {
  ...
  const test = await templatesApi.listTemplates(args.accountId, {
    templateId: "f9acf5ca-f819-4615-94ff-9e30ca1369b6",
  });
  try {
...
function makeTemplate(args) {
  ...
  let docPdfBytes,
    docs = path.resolve(demoDocsPath, args.docFile);
  //let doc2File = path.resolve(demoDocsPath, args.docFile);
  // read file from a local directory
  // The reads could raise an exception if the file is not available!
  try {
    docPdfBytes = fs.readFileSync(docs);
  } catch (error) {
    console.log({ error });
  }
